...
record further material decisions; D32–D34 (added by #89) record the design-v2
direction and the derived-data scope. Status: ✅ decided · 🟡 provisional
(named trigger will confirm or overturn). **D31 is reserved for a concurrent
branch** — #89 started at D32 deliberately to avoid a merge collision, so a
gap at D31 is not an omission.
